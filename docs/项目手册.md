...
11. [百家号发布流程详解](#11-百家号发布流程详解)
12. [已知问题与解决方案](#12-已知问题与解决方案)
13. [后续开发计划](#13-后续开发计划)
14. [附录D：更新日志](#附录d更新日志)
...
| **账号删除功能** | 支持删除账号，同时清理登录信息和浏览器配置 |
| **账号昵称编辑** | 双击账号名称可手动编辑，解决昵称获取失败的情况 |
| **账号拖拽排序** | 拖拽行号可调整账号顺序，控制发布顺序 |
| **独立浏览器** | 一键打开带登录状态的独立浏览器，方便手动操作 |
| **账号昵称显示** | 登录后自动获取并显示平台昵称 |
| **多平台支持** | 支持今日头条、搜狐号、百家号，可扩展其他平台 |
...
│  │            平台适配器层                                  │                 │
│  │  ┌──────────────┐  ┌──────────────┐  ┌────────────────┐ │                 │
│  │  │ToutiaoAdapter│  │ SohuAdapter  │  │BaijiahaoAdapter│ │                 │
│  │  │  今日头条     │  │   搜狐号      │  │    百家号       │ │                 │
│  │  └──────────────┘  └──────────────┘  └────────────────┘ │                 │
│  └──────────────────────────────────────────────────────────┘                 │
└──────────────────────────────────────────────────────────────┘
...
| **BaijiahaoAdapter** | `src/adapters/baijiahao_adapter.py` | 百家号平台适配器 |
...
| `cancel()` | 取消任务执行 |
| `reset()` | 重置调度器状态 |
| `add_account(platform)` | 动态添加新账号 |
| `remove_account(account_id)` | 删除账号 |
| `reorder_accounts(new_order)` | 重新排序账号列表 |
...
| `COVER_SELECT_BTN` | `.article-cover-images-wrap .article-cover-images > div > div > div > div` | 封面选择按钮（点击打开素材抽屉） |
| `COVER_SELECT_AREA_ALT` | `.article-cover-images-wrap` | 备用封面选择器 |
| `MY_MATERIAL_TAB` | `text=我的素材` | "我的素材"标签页（文本定位） |
| `MATERIAL_FIRST_IMAGE` | `.byte-drawer .img-span` | 素材库第一张图片 |
| `MATERIAL_CONFIRM_BTN` | `div.byte-drawer .footer button.byte-btn-primary` | 素材选择确定按钮 |
| `PUBLISH_BTN` | `button.publish-btn-last` | 预览并发布按钮 |
| `CONFIRM_PUBLISH_BTN` | `button.publish-btn-last` | 确认发布按钮 |

#### 选择器获取方法

1. 使用浏览器开发者工具 (F12) 检查元素
2. 右键元素 → Copy → Copy selector
3. 简化选择器，保留关键定位信息
4. 优先使用 class 选择器，避免过于复杂的层级结构
...
2. 填写标题
   ...
   └── random_delay(3, 5)
        ...
        ▼
3. 填写正文
   └── fill(CONTENT_EDITOR, content)
   └── random_delay(4, 6)
        │
        ▼
4. 选择封面（从素材库）
   ├── click(COVER_SELECT_BTN)        ← 打开素材抽屉
   │   └── random_delay(3, 4)         ← 等待抽屉动画完成
   ├── click("我的素材")              ← 文本定位
   │   └── random_delay(3, 4)         ← 等待素材加载
   ├── click(MATERIAL_FIRST_IMAGE)    ← 选择第一张图片
   │   └── random_delay(2, 3)
   └── evaluate("el.click()")         ← JS点击确定按钮（避免视口问题）
       └── random_delay(3, 4)
        │
        ▼
5. 点击发布按钮
...
| 页面 | URL |
|-----|-----|
| 登录页 | `https://baijiahao.baidu.com/builder/rc/login` |
| 后台首页 | `https://baijiahao.baidu.com/builder/rc/noticemessage/notice_system` |
| 发布页 | `https://baijiahao.baidu.com/builder/rc/edit?type=news` |

### 11.2 元素选择器（BaijiahaoSelectors类）

所有选择器定义在 `src/adapters/baijiahao_adapter.py` 的 `BaijiahaoSelectors` 类中：

> **最后更新：2025-12-23**

| 选择器名称 | 用途 |
|-----------|------|
| `PUBLISH_WORK_BTN` | "发布作品"按钮 |
| `TITLE_INPUT` | 标题输入框 |
| `CONTENT_EDITOR` | 正文编辑器（UEditor，位于iframe内） |
| `SINGLE_IMAGE_RADIO` | "单图"封面选项 |
| `IMAGE_SELECT_BOX` | 图片选择框 |
| `CONFIRM_BTN` | 确认按钮 |
| `PUBLISH_BTN` | 发布按钮 |
...
4. 填写标题
   ...
   └── random_delay(2, 3)
        │
        ▼
5. 填写正文（iframe内的UEditor）
   └── frame_locator("#ueditor_0").locator("body").fill(content)
   └── random_delay(2, 3)
        │
        ▼
6. 选择"单图"封面选项
   └── click(SINGLE_IMAGE_RADIO)
   └── random_delay(1, 2)
        │
        ▼
7. 点击图片选择框
   └── click(IMAGE_SELECT_BOX)
   └── random_delay(2, 3)
        │
        ▼
8. 点击"免费正版图库"标签
   └── 多选择器策略（rc-tabs-0-tab-rights / rc-tabs-1-tab-rights）
   └── random_delay(1, 2)
        │
        ▼
9. 搜索图片关键词
...
百家号使用UEditor富文本编辑器，正文编辑区域位于iframe内：

```python
# 正文编辑需要先获取iframe，再操作内部元素
ueditor_frame = page.frame_locator("#ueditor_0")
body_element = ueditor_frame.locator("body")
await body_element.fill(content)
```

#### 新功能引导弹窗

百家号编辑页面会弹出新功能引导，需要主动关闭：
...
### 2025-12-17 - 今日头条选择器修复

**问题描述**：
今日头条发布流程中，封面选择和发布按钮的元素定位不准确，导致发布失败。

**修复内容**：

1. **更新选择器** (`src/adapters/toutiao_adapter.py`)：

   | 元素 | 旧选择器 | 新选择器 |
   |------|---------|---------|
   | 封面选择按钮 | `.article-cover-images-wrap .upload-image-panel` | `.article-cover-images-wrap .article-cover-images > div > div > div > div` |
   | 我的素材标签 | `.byte-drawer .byte-tabs-header-nav...` | `text=我的素材` |
   | 素材第一张图片 | `.byte-drawer .ReactVirtualized__List...` | `.byte-drawer .img-span` |
   | 确定按钮 | `.byte-drawer button.byte-btn-primary` | `div.byte-drawer .footer button.byte-btn-primary` |
   | 发布按钮 | `#root button.publish-btn.publish-btn-last > span` | `button.publish-btn-last` |

2. **优化点击策略**：
   - 确定按钮使用 JavaScript 点击 (`evaluate("el.click()")`)，避免元素在视口外无法点击的问题

3. **调整等待时间**：
   - 打开素材抽屉后等待 3-4 秒（原 2-3 秒），确保动画完成
   - 点击"我的素材"后等待 3-4 秒，确保素材加载完成

**测试结果**：
使用"月月说流量"账号测试，全部8个步骤通过，单篇文章发布耗时约 45-65 秒。

---

### 2025-12-22 - 账号删除功能 + 独立浏览器功能

**新增功能**：

1. **账号删除功能**
   - 在账号表格的「操作」列添加「🗑️」删除按钮
   - 点击后弹出确认对话框，防止误删
   - 删除时同时清理：
     - `accounts.json` 中的账号配置
     - `browser_profiles/{profile_dir}` 目录（包含登录Cookie）
     - 内存中的账号任务列表
   - 发布任务进行中时禁止删除

2. **独立浏览器功能**
   - 在账号表格的「操作」列添加「🌐」打开浏览器按钮
   - 点击后打开一个**独立浏览器**，自动加载该账号的登录状态（Cookie）
   - 浏览器**不受程序管理**，用户可以一直使用，手动关闭
   - 自动导航到平台首页：
...
| 文件 | 修改内容 |
|------|---------|
| `src/utils/config.py` | 添加 `delete_account()` 方法 |
| `src/core/scheduler.py` | 添加 `remove_account()` 方法 |
| `src/browser/browser_manager.py` | 添加 `open_standalone_browser()` 方法 |
| `src/ui/main_window.py` | 调整UI布局（4列），添加操作按钮和处理方法 |

**UI变更**：
- 账号表格从3列改为4列：选择 | 账号名称 | 登录 | 操作
- 「操作」列包含「🌐」和「🗑️」两个按钮

---

### 2025-12-23 - 百家号自动发布 + 账号编辑排序功能
...
| 文件 | 修改内容 |
|------|---------|
| `src/adapters/baijiahao_adapter.py` | 实现完整的百家号发布流程，优化登录检测 |
| `src/ui/main_window.py` | 添加账号双击编辑、拖拽排序功能 |
| `src/utils/config.py` | 添加 `update_account_name()` 和 `reorder_accounts()` 方法 |
| `src/core/scheduler.py` | 添加 `reorder_accounts()` 方法 |

**百家号发布流程**（v0.2优化）：

```
进入后台首页 → 点击发布作品 → 关闭引导弹窗 → 填写标题 → 填写正文(iframe)
     → 选择单图封面 → 点击免费正版图库 → 搜索"渡鸦" → 选择图片（点击图片）
     → 点击确认 → 等待10秒 → 点击发布
```

**UI变更**：

- 账号面板标题更新为"账号列表 (双击名称可编辑，拖拽可排序)"
- 账号表格支持拖拽排序（通过行号拖拽）
- 账号名称列支持双击编辑
- 新增「➕ 增加百家号账号」按钮（粉红色）
...
- 删除多余的"点击图片选择框"步骤
- 优化图片选择：直接点击图片本身（使用force=True模拟真人点击）
- 更新选择器适配最新百家号界面：
  - 图片选择器：`#rc-tabs-1-panel-authLib .pubu-content > div:nth-child(5) > img`
  - 确认按钮：`[class*='authLib'] [class*='bottom'] button.cheetah-btn-primary`
  - 发布按钮：`#root .op-list-right > div:nth-child(4) > button > span`
