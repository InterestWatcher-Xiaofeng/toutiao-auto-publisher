...
| 平台 | 每账号每日发布数量 | 说明 |
|-----|------------------|------|
| **今日头条** | 10篇 | 每个头条账号每天发布10篇文章 |
| **搜狐号** | 5篇 | 每个搜狐账号每天最多发布5篇文章 |

**示例场景**（3个头条账号 + 2个搜狐账号）：

```
今日头条-账号1 → 发布文章1-10   （10篇）
今日头条-账号2 → 发布文章11-20  （10篇）
今日头条-账号3 → 发布文章21-30  （10篇）
搜狐号-账号1   → 发布文章31-35  （5篇）
搜狐号-账号2   → 发布文章36-40  （5篇）
────────────────────────────────
总计: 40篇文章
```
...
```json
{
  "accounts": [
    {
      "account_id": "toutiao_1",
      "account_name": "今日头条-账号1",
      "platform": "toutiao",
      "profile_dir": "data/browser_profiles/toutiao_account1"
    },
    {
      "account_id": "sohu_1",
      "account_name": "搜狐号-账号1",
      "platform": "sohu",
      "profile_dir": "data/browser_profiles/sohu_account1"
    }
  ]
}
```
...
**标题输入框**:
```css
#root > div > div.left-column > div > div.publish-editor >
div.publish-editor-title-wrapper > div > div > div.title-wrapper >
...
```css
#root > div > div.left-column > div > div.publish-editor >
div.syl-editor-wrap > div > div.ProseMirror
```

**选择封面按钮**:
```css
.cover-editor-main .upload-btn
```
...
3. 点击标题区域关闭弹窗
   └── click(标题输入框, force=True)
        ...
        ▼
4. 清空并输入标题
   └── fill(标题输入框, article.title)
        │
        ▼
5. 输入正文
   └── fill(正文编辑器, article.content)
        │
        ▼
6. 选择封面（从素材库）
   ├── click("选择封面")
...
**解决方案**: 使用 `force=True` 参数强制点击，即使有元素遮挡也能点击成功。
